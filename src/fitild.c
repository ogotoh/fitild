...
static	const	double*	Initial_mn_step = Frechet_mn_step;
static	const	GslAlg	gslalg[] = {BFGS2, NMSIMP1, CONJ_FR, ASIS};
...
void usage()
{
	...
	fputs("\t-c#\t: degree of convergence (1e-4)\n", stderr);
...
static	const	char*	graph_out = 0;
	...
	while (--argc && **++argv == '-') {
	  ...
	  switch (*gsl_alg) {
		...
		break;
	    case 'c': if ((pn = getarg(argc, argv, true)))
		gslprm.mn_conv = atof(pn); break;
	    ...
	    case 'k': if ((pn = getarg(argc, argv, true)))
		minsamples = atoi(pn); break;
	    case 'l': if ((pn = getarg(argc, argv, true)))
		lildprm.llmt = atof(pn); break;
	    ...
	    case 'r': if ((pn = getarg(argc, argv, true)))
		gslprm.opt_max_iter = atoi(pn); break;
	    case 's': if ((pn = getarg(argc, argv, true)))
		gslprm.mn_step = atof(pn); break;
	    case 'u': if ((pn = getarg(argc, argv, true)))
		lildprm.ulmt = atof(pn); break;
	    case 'x': if ((pn = getarg(argc, argv, true)))
		lildprm.ndiv = atoi(pn); break;
	    case 'F': defpdf = FRECHET; pdfchar = 'F'; 
		Initial_mn_step = Frechet_mn_step; break;
	    case 'G': defpdf = GEOMETRIC; pdfchar = 'G'; 
		Initial_mn_step = Geometric_mn_step; break;
	    case 'L': if ((pn = getarg(argc, argv, true)))
		lildprm.minx = atoi(pn); break;
	    case 'N': defpdf = LOGNORMAL; pdfchar = 'N';
		Initial_mn_step = LogNormal_mn_step; break;
	    case 'P': if ((pn = getarg(argc, argv, true)))
		lildprm.psdcnt = atof(pn); break;
	    case 'V': verbs = (pn = getarg(argc, argv, true))?
		atoi(pn): 3; break;
	    case 'U': if ((pn = getarg(argc, argv, true)))
		lildprm.maxx = atoi(pn); break;
	    default: break;
	  }
	}
