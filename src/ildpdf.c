...
*
*	Copyright(c) Osamu Gotoh <<o.gotoh@aist.jp>>
*****************************************************************************/
...
GslPrm	gslprm = {1000, 1000, 100, 0., 1.e-3, 0.01, 1.e-4, 1.e-3};
bool	plot_cdf = false;
...
IldPrm::IldPrm(int np, const char** argv, StatDist pdf)
	: ildpdf(pdf), m_size(NoDistParam[ildpdf]), vrtl(false), sname(0), 
	  n_param(np), sid(0), n_sample(0), k_th_qtil(0), cdf_table(0)
{
...
IldPrm::IldPrm(Ild* inst, StatDist pdf)
	: ildpdf(pdf), n_modes(1), m_size(NoDistParam[ildpdf]), vrtl(false),
	  sname(0), n_param(m_size - 1), sid(0), n_sample(int(inst->ftotal)),
	  k_th_qtil(0), cdf_table(0)
{
	dparam[n_param] = 1.;
	if (ildpdf == GEOMETRIC) {
	    dparam[1] = inst->minx();
...
IldPrm::IldPrm(const char* ip_stat, const char* gs)
	: ildpdf(FRECHET), n_modes(0), m_size(NoDistParam[ildpdf]), vrtl(false), 
...
double IldPrm::pdf_function(double len)
{
...
double IldPrm::cumulative(double len)
{
	double	f = 0;
	for (int j = 0; j < n_param; j += m_size) {
	    const double&	mu = dparam[j];
...
bool IldPrm::proper()
{
	...
	for (int j = 0; j < n_param; j += m_size) {
	    if (ildpdf == FRECHET) {
		rv = rv && (0 <= dparam[j + 3] && dparam[j + 3] <= 1.)
		    && dparam[j + 2] >= 0;
	    } else if (ildpdf == LOGNORMAL) {
		rv = rv && (0 <= dparam[j + 2] && dparam[j + 2] <= 1.);
	    } else if (ildpdf == GEOMETRIC) {
		rv = rv && (0 <= dparam[j] && dparam[j] <= 1.);
	    }
	}
	return rv;
}
...
char* IldPrm::strget(char* str, char** terms)
{
	...
	if (!(qs = car(ps)) || !*++ps) return (0); min_x = atoi(qs);
	if (!(qs = car(ps)) || !*++ps) return (0);
	if (!(qs = car(ps)) || !*++ps) return (0); max_x = atoi(qs);
	if (!(qs = car(ps)) || !*++ps) return (0);
	int	n = 0;
...
// generage a uniform distribution [0, 1)
	...
	int	n = 0;
	double	p = 0.;
	Dhash<int, double>	lf(ildprm->max_x);
...
void GnuPlotLild::initialize()
{
	...
	*data = new double[n_clmns * (n_rows + 2)] + 1;
	...
	if (invtransform) {
	    int 	px = xval[-1];
	    data[1][-1] = px;
	    data[0][-1] = transform(double(px));
	    for (int n = 0; n <= n_rows; ++n) {
		int	ix = xval[n];
		if ((ix - px) == 1) {
		    data[1][n] = ix;
		    data[0][n] = transform(data[1][n]);
		} else {
		    ++px;
		    data[0][n] = (transform(double(px)) + transform(double(ix))) / 2;
...
void GnuPlotLild::add(IldPrm* dprm)
{
	for (int n = 0; n <= n_rows; ++n) {
	    double	dx = data[1][n];
	    if (plot_cdf)
		data[clmn][n] = dprm->cumulative(dx);
	    else 
		data[clmn][n] = normal_factor * dx * dprm->pdf_function(dx);
	}
...
GnuPlotLild::GnuPlotLild(IldPrm* dprm, int np, Lild** lild, int ni)
...
GnuPlotLild::GnuPlotLild(IldPrm* dprm, int np, Ild** ilds, int ni)
	: PutIntoBins(lildprm.ndiv, lildprm.llmt, lildprm.ulmt, 0, 0, false),
	  n_lild(ni), n_fitf(np), data(0)
{
	initialize();
...
void GnuPlotLild::plot(const char* oform, Strlist* iname, StrHash<int>* phyl_code)
{
static	const	char	pdffmt[] = "lines lw 3 lc rgb '%s'";
static	const	char	smlfmt[] = "%s ps 1.5 lc rgb '%s'";
const	char*	smllt = n_fitf? "points": "linespoints";
	FILE*	fd = fopen(gpdata, "w");
	if (!fd) fatal("Can't write to %s !\n", gpdata);
const	char**	header = new const char*[2 + n_fitf + clmn];
	fputs("loglen\tlen", fd);
	int	c = 2;
	for (int i = 0; i < n_fitf; ++i, ++c)
	    fprintf(fd, "\t%s", header[c] = sname[i]);
	if (iname) {
	    for (int i = 0; i < n_lild; ++i, ++c)
	...
	fprintf(fd, "title '%s' with ", header[2]);
	if (n_fitf)
	    fprintf(fd, pdffmt, "black");
	else
	    fprintf(fd, smlfmt, smllt, "black");
	c = 3;
	for (int i = 1; i < n_fitf; ++i) {
	    fprintf(fd, ",\\\n'' using 1:($%d) ", ++c);
	    fprintf(fd, "title '%s' with ", header[c - 1]);
	    int	cc = c;
	    if (phyl_code) {
		KVpair<INT, int>* kv = phyl_code->find(header[c - 1]);
		if (kv) cc = kv->val + 2;
	    }
	    fprintf(fd, pdffmt, colors[cc % n_colors + 1]);
	}
	for(int i = 2; c < clmn; ++i) {
	    fprintf(fd, ",\\\n'' using 1:($%d) ", ++c);
	    fprintf(fd, "title '%s' with ", header[c - 1]);
	    int	cc = i;
	    if (phyl_code) {
