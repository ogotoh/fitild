...
enum	StatDist {FRECHET, LOGNORMAL, GEOMETRIC};
...
static	const	int	NoDistParam[] = {4, 3, 3};
...
static	const	double	max_kappa = 30;
...
class SpecList : public StrHash<int> {
	int	n_spec;
public:
	SpecList(int ac, const char** av) : StrHash<int>(max(ac, min_hash)) {
	    const char**	av0 = av;
	    while (ac-- && **av != '-') incr(*av++);
	    n_spec = av - av0;
	}
...
const	char*	fname;
	...
	Ild(IldPrm* ildprm, int n_sample, Drand48_data* drand_buff, const char* snm = 0);
#else
	Ild(Ild* src);	// bootstrap resampling
	Ild(IldPrm* ildprm, int n_sample, const char* snm = 0);	// simulated ild
#endif
	~Ild();
...
class GnuPlotLild : public PutIntoBins {
	...
	void	initialize(); 
	void	add(IldPrm* dprm);
	void	add(Ild* lild);
	void	add(Lild* lild);
public:
	Strlist	sname;
	void	plot(const char* oform = 0, Strlist* iname = 0, StrHash<int>* phyl_code = 0);
	...
	GnuPlotLild(IldPrm* dprm, int nprm, Lild** lild, int nild = 1);
	GnuPlotLild(IldPrm* dprm, int nprm, Ild** ilds, int nild = 1);
...
{
	return (p[2] > 1? (p[0] + p[1] * gsl_sf_gamma(1 - 1 / p[2])): 0);
}
