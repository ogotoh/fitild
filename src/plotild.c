...
void usage()
{
	fputs("Usage:\n\tplotild [options] [y1.ild y2.ild ...] [-d IpTable x1 x2 ...]\n", stderr);
	fputs("\nOptions:\n", stderr);
	...
	fputs("\t-If\t: linear scale X-axix (false)\n", stderr);
	fputs("\t-L#\t: shortest intron length (0)\n", stderr);
	fputs("\t-G\t: geometric distribution (Frechet)\n", stderr);
	fputs("\t-N\t: lognormal distribution (Frechet)\n", stderr);
...
void getoptions(int& argc, const char**& argv)
{
	...
	while (--argc && **++argv == '-') {
	  switch (argv[0][1]) {
	    case 'a': if ((pn = getarg(argc, argv, true)))
		 alpha = atof(pn); break;
	    ...
	    case 'F': defpdf = FRECHET; break;
	    case 'G': defpdf = GEOMETRIC; break;
	    case 'I': xlogscale = false; break;
	    case 'L': if ((pn = getarg(argc, argv, true)))
		lildprm.minx = atoi(pn); break;
	    case 'N': defpdf = LOGNORMAL; break;
	    case 'U': if ((pn = getarg(argc, argv, true)))
...
int main(int argc, const char *argv[])
{
	...
	if (xlogscale) {
	    GnuPlotLild	gp(iprm, nprm, lild, nild);
	    gp.plot(graph_out, 0, phyl_code);
	} else {
	    GnuPlotLild	gp(iprm, nprm, ilds, nild);
	    gp.plot(graph_out, 0, phyl_code);
	}
	delete[] iprm;
	if (lild) {
